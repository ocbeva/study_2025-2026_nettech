...
Указав к ipconfig ключ /all, мы получим более подробную информацию, в том числе физический адрес (mac). В нашем случае mac адрес имеет вид 74-04-f1-d0-d0-45. Первые 3 байта - это идентификатор производителя, а остальные 3 - интерфейса. В первом байте (74) зашита информация о том, что адрес индивидуальный, и что является глобально администрируемым, поскольку последние 2 бита 74 в двоичной системе, если переводить из hex, равны нулям (рис. [-@fig:002]).
...
Попробуем пинганутся к шлюзу, адрес которого мы получили в самом начале (рис. [-@fig:005]).
...
Посмотрим теперь на arp пакеты. Как видим, в них не так много информации. Они отвечают за получение физического адреса. Так, мы видим, что первый arp пакет пытается получить адрес нашего устройства (рис. [-@fig:009]).
...
Теперь пинганём яндекс (рис. [-@fig:011]).
...
Входящий пакет же будет отличаться тем, что длина сегмента и чексумма будут другие, а порты поменяются местами (рис. [-@fig:018]).
...
Сервер, как бы, протягивает руку, давая ответ в виде syn и ack, в котором как бы говорит, что теперь знает о клиенте (рис. [-@fig:024]).

![syn + ack](image/24.png){#fig:024 width=70%}

Клиенту остаётся пожать руку, сказав серверу, что тоже теперь о нём знает, при этом флаг syn больше не стоит, поскольку оба они теперь знают о друг друге, и запрос на синхронизацию больше не нужен (рис. [-@fig:025]).
